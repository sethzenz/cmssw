/**
 *  @file   RecoParticleFlow/PandoraTranslator/src/MipPhotonSeparationAlgorithm.cc
 * 
    ...
    unsigned int showerEndLayer) const
{
    ...
    if ((NULL != pMipCluster) && (NULL != pPhotonCluster) && (pPhotonCluster->GetNCaloHits() >= m_minHitsInPhotonCluster))
    {
        ...
        const reco::PFRecTrack* pftrack = static_cast<const reco::PFRecTrack*>(pTrack->GetParentTrackAddress());
        ...
        const float trackEnergyError2(deltaE*deltaE);
        const float originalChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), pOriginalCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergy, trackEnergyError2));
        const float newChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), pMipCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergy, trackEnergyError2));
