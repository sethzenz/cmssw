/**
 *  @file   RecoParticleFlow/PandoraTranslator/src/MainFragmentRemovalAlgorithmFast.cc
 * 
...
using cms_content::ReclusterHelper;
using lc_content_fast::build_3d_kd_search_region;

namespace cms_content_fast
{
    ...
    float &globalDeltaChi2) const
{
    ...
    float totalTrackEnergy(0.f), totalClusterEnergy(0.f), totalTrackEnergyError2(0.f);
        ...
        ++iter)
    {
        ...
        const float parentTrackEnergyError2(chargedClusterContact.GetParentTrackEnergyError2());
        const float parentClusterEnergy(chargedClusterContact.GetParentCluster()->GetTrackComparisonEnergy(this->GetPandora()));

        const float oldChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), parentClusterEnergy, parentTrackEnergy, parentTrackEnergyError2));
        const float newChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), daughterClusterEnergy + parentClusterEnergy, parentTrackEnergy, parentTrackEnergyError2));
        ...
        totalTrackEnergyError2 += parentTrackEnergyError2;
    ...
    const float oldChiTotal(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), totalClusterEnergy, totalTrackEnergy, totalTrackEnergyError2));
    const float newChiTotal(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), daughterClusterEnergy + totalClusterEnergy, totalTrackEnergy, totalTrackEnergyError2));
    ...
    const unsigned int correctionLayer, const float globalDeltaChi2)
{
    ...
    const float oldChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), parentCorrectedClusterEnergy, parentTrackEnergy, parentTrackEnergyError2));
    const float newChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), daughterCorrectedClusterEnergy + parentCorrectedClusterEnergy, parentTrackEnergy, parentTrackEnergyError2));
    ...
    const Parameters &parameters)
{
    ...
    m_parentTrackEnergyError2 = trackEnergyErrorSum2;
}
